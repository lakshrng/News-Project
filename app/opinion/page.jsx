...
export default function OpinionPage() {
  ...
  const fetchOpinionNews = async () => {
    try {
      setLoading(true);
      const resp = await axios.get('/api/news?q=opinion editorial India&limit=20');
      if (resp.data?.success) {
        setArticles(resp.data.articles || []);
      }
    } catch (err) {
      console.error('Error loading opinion news:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="section">
      ...
      <p className="subtitle">Editorials, columns, and expert opinions</p>
          ...
          {articles.map((article, i) => {
            ...
            return (
              <article className="news-card" key={i}>
                ...
                <span className="category-label">OPINION</span>
                <h3>
